...
import { isDirty, MSG_SIN_GUARDAR } from './utils/dirty';
...
export default function AppLayout() {
  ...
  // y volver arriba (en movil, la lista o el cuadro de mando dejan el scroll abajo)
  useEffect(() => { loadEmisor(); window.scrollTo(0, 0); }, [location.pathname]);

  // Ajustes avisa al guardar: refrescar nombre/logo del sidebar sin navegar
  useEffect(() => {
    const onChanged = () => { loadEmisor(); };
    window.addEventListener('presufact-emisor-changed', onChanged);
    return () => window.removeEventListener('presufact-emisor-changed', onChanged);
  }, []);

  // Salir del editor con cambios sin guardar pide confirmacion
  const guardDirty = (e) => {
    if (isDirty() && !window.confirm(MSG_SIN_GUARDAR)) e.preventDefault();
  };
  ...
  const navItem = ({ to, label, icon: Icon, end }, opts = {}) => (
    <NavLink key={to} to={to} end={end}
      onClick={(e) => { guardDirty(e); if (!e.defaultPrevented) opts.onClick?.(); }}
  ...
  const brand = (
    <button onClick={() => { if (isDirty() && !window.confirm(MSG_SIN_GUARDAR)) return; navigate('/app'); setMobileOpen(false); }}
